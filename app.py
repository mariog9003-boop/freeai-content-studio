...
def run_semantic_ai_engine(text_input, style_preset):
    ...
    p, s, e = sentences[0], sentences[1] if len(sentences) > 1 else sentences[0], sentences[2] if len(sentences) > 2 else sentences[0]
    
    if "TikTok" in style_preset or "Shorts" in style_preset or "IG" in style_preset or "Retention" in style_preset:
        return f"🎬 **SHORT-FORM VIDEO SCRIPT**\n\n🎥 **Visual Frame 1 (0-3s):** Fast flashing high-contrast neon text.\n🎙️ **Audio Hook:** \"Stop wasting time trying to manually figure out how {p.lower()}! Here is the brutal truth...\"\n\n🎥 **Visual Frame 2 (3-10s):** Zoom in tight on results panel.\n🎙️ **Audio Body:** \"Most fail because they track basic parameters. The data reveals that {s.lower()}. When {e.lower()}, workflows shatter.\"\n\n🎥 **Visual Frame 3 (10-15s):** Smooth pan to bio link.\n🎙️ **Audio Outro:** \"Drop a follow to lock in your daily leverage loop. Click my bio node right now.\""
    elif "LinkedIn" in style_preset or "Thread" in style_preset:
        return f"💼 **HIGH-AUTHORITY VALUE POST**\n\nI used to think standard organizational methods worked. I was entirely wrong.\n\nHere is the real data behind how {p.lower()}:\n\n• **1. Core Catalyst:** {s}.\n• **2. The Underappreciated Shift:** When {e.lower()}, it creates un-matched leverage.\n\nTraditional operators will be out-paced by automation before the end of this year. 👇 #Growth #Innovation"
    elif "Sales" in style_preset or "Framework" in style_preset:
        return f"💰 **PERSUASIVE CONVERSION MATRIX**\n\nAre you still leaving conversions up to chance using outdated frameworks?\n\nIt is leaking revenue every single hour. Here is your unfair advantage: **{p}**.\n\nThis isn't an update—it is a survival blueprint. Research verifies that {s.lower()}. When you ignore that {e.lower()}, attention drops by 80%.\n\n👉 Click the secure checkout link below to unlock your unrestricted lifetime access slot right now."
    else:
        return f"✨ **EXECUTIVE SUMMARY LOGIC**\n\n• 🧬 **Primary Vector:** {p}.\n\n• ⚡ **Dynamic Node:** {s}.\n\n• 🧪 **Supporting Context:** {e}."
